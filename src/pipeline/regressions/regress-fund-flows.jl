...
function regress_fund_flows(model_name; filter_by=nothing) # model_name = :ff_usa_ff3; filter_by = x->(investment_target_is(x, :usa) .&& bho_dates_only(x))
    task_start = time()
...
###
function initialise_flow_data2(model_name)
